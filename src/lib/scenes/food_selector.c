    ...
    for (i =0; i <= HID_input_events_queue_depth; ++i) // For every event in the event queue;
    {
        ...
        switch (this_event){
            case(BUTTON_A_PRESS): // Scroll up the list but one per keypress.
               SCENE_CURSOR_POS -= 1;
                if (SCENE_CURSOR_POS < 0){
                    SCENE_CURSOR_POS = FOODSEL_active_lines;
            ...
            case(BUTTON_B_PRESS): // scroll down the list by one per keypress
		        SCENE_CURSOR_POS += 1;
		        if (SCENE_CURSOR_POS >= FOODSEL_active_lines){
		            SCENE_CURRENT_PAGE += 1;
		            SCENE_CURSOR_POS = 0;
            ...
            case(BUTTON_C_PRESS): //Make a selection of the current line.
                ...
                if (SCENE_CURSOR_POS + (SCENE_CURRENT_PAGE*FOODSEL_active_lines) < target_count_opts){
                ...
                break;
            case(BUTTON_D_PRESS): // exit back to the feeding types menu
                SCENE_EXIT_FLAG = true;
...
char* FOODSEL_computeLine(int line_number, char ** options, int count_opts){
    if (line_number + (SCENE_CURRENT_PAGE*FOODSEL_active_lines) < count_opts){ // The requested index is in-bound and we can do this.
        return options[line_number + (SCENE_CURRENT_PAGE*FOODSEL_active_lines)];
    }
    else {
        return "";
    }
...
void SCENE_FoodMenu(char * target_LSTRING_HEADER, char ** target_LARRAY, Food * target_FARRAY, int target_count_opts){
    SCENE_PAGE_COUNT = FOODSEL_computePagination(target_count_opts);
